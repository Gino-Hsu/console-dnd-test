...
import { applyEditLogAction } from '@/lib/layout/reducer';
import {
  getCurrentPage,
  getPublishedPage,
  appendEditLog,
...
export function useLayoutEditor() {
  ...
  const performLog = useCallback((operation: EditOperation, immediate: boolean) => {
    if (immediate) {
  ...
  const applyMove = useCallback(
    (
      ...
      setLayoutsWithLog(
        prev => moveItem(prev, activeId, targetContainer, index ?? undefined),
        'move-layout',
        '移動項目',
        true,
